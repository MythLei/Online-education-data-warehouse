...
insert overwrite table ${APP}.dwd_interaction_review_add_inc partition (dt='$do_date')
select data.id,
       data.user_id,
       data.course_id,
       data.review_stars,
...
insert overwrite table ${APP}.dwd_interaction_comment_add_inc partition (dt='$do_date')
select data.id,
       data.user_id,
       data.chapter_id,
       data.course_id,
...
insert overwrite table ${APP}.dwd_play_user_chapter_process_inc partition (dt='$do_date')
select data.id,
       data.user_id,
       data.course_id,
       data.chapter_id,
...
insert overwrite table ${APP}.dwd_exam_question_exam_inc partition (dt='$do_date')
select data.id,
       data.exam_id,
       data.paper_id,
       data.question_id,
...
insert overwrite table ${APP}.dwd_exam_paper_exam_inc partition (dt='$do_date')
select data.id,
       data.user_id,
       data.paper_id,
       date_format(data.create_time, 'yyyy-MM-dd') date_id,
...
insert overwrite table ${APP}.dwd_play_video_play_inc partition (dt='$do_date')
select video_id
     , t1.user_id
     , t1.chapter_id
     , course_id
...
case $1 in
    ...
    "all" )
        hive -e "$dwd_interaction_comment_add_inc$dwd_interaction_review_add_inc$dwd_interaction_favor_add_inc$dwd_exam_question_exam_inc$dwd_exam_paper_exam_inc$dwd_trade_cart_add_inc$dwd_trade_order_detail_inc$dwd_trade_pay_detail_suc_inc$dwd_traffic_page_view_inc$dwd_user_login_inc$dwd_user_register_inc$dwd_play_video_play_inc$dwd_play_user_chapter_process_inc"
    ;;
esac
